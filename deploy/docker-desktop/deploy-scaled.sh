#!/usr/bin/env bash
# Deploy orin in scaled mode (separate apiserver, controller, reposerver)
...
if kubectl get statefulset orin-postgres -n "${NS}" &>/dev/null; then
...
kubectl rollout status statefulset/orin-postgres -n "${NS}" --timeout=120s
...
type: Opaque
stringData:
  database-url: "postgres://k8sui:k8sui@orin-postgres:5432/k8sui?sslmode=disable"
...
sed "s|image: orin:dev|image: ${IMAGE}|g; s|args: \[\"reposerver\"\]|args: [\"reposerver\"]|g" \
  "${ROOT}/deploy/scaled/03-reposerver.yaml" | \
  kubectl apply -f -

echo "==> Deploying Controller (leader election)"
sed "s|image: orin:dev|image: ${IMAGE}|g" \
...
kubectl rollout status deployment/orin-reposerver -n "${NS}" --timeout=120s || true
kubectl rollout status deployment/orin-controller -n "${NS}" --timeout=120s || true
kubectl rollout status deployment/orin-apiserver -n "${NS}" --timeout=120s
...
echo ""
echo "Components:"
kubectl get pods -n "${NS}" -l app.kubernetes.io/name=orin-apiserver -o wide 2>/dev/null || true
kubectl get pods -n "${NS}" -l app.kubernetes.io/name=orin-controller -o wide 2>/dev/null || true
kubectl get pods -n "${NS}" -l app.kubernetes.io/name=orin-reposerver -o wide 2>/dev/null || true
echo ""
echo "Access:"
echo "  kubectl port-forward -n ${NS} svc/orin-apiserver ${PF_PORT}:80"
